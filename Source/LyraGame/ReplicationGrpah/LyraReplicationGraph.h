...
public:
	...
	virtual void InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection) override; // 새로운 연결(플레이어)이 들어올 때마다 해당 연결을 위한 전용 노드를 생성합니다.
	...
	UPROPERTY()
	TObjectPtr<UReplicationGraphNode_ActorList> AlwaysRelevantNode; // 항상 연관된 액터들을 관리하는 전역 리스트 노드입니다.
