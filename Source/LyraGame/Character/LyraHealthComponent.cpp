...
float ULyraHealthComponent::GetArmor() const
{
	return (HealthSet ? HealthSet->GetArmor() : 0.0f);
}

float ULyraHealthComponent::GetMaxArmor() const
{
	return (HealthSet ? HealthSet->GetMaxArmor() : 0.0f);
}

float ULyraHealthComponent::GetArmorNormalized() const
{
	if (HealthSet)
	{
		const float Armor = HealthSet->GetArmor();
		const float MaxArmor = HealthSet->GetMaxArmor();
...
void ULyraHealthComponent::HandleArmorChanged(AActor* DamageInstigator, AActor* DamageCauser, const FGameplayEffectSpec* DamageEffectSpec, float DamageMagnitude, float OldValue, float NewValue)
{
	OnArmorChanged.Broadcast(this, OldValue, NewValue, DamageInstigator);
}
void ULyraHealthComponent::HandleMaxArmorChanged(AActor* DamageInstigator, AActor* DamageCauser, const FGameplayEffectSpec* DamageEffectSpec, float DamageMagnitude, float OldValue, float NewValue)
{
	OnMaxArmorChanged.Broadcast(this, OldValue, NewValue, DamageInstigator);
}
void ULyraHealthComponent::HandleOutOfArmor(AActor* DamageInstigator, AActor* DamageCauser, const FGameplayEffectSpec* DamageEffectSpec, float DamageMagnitude, float OldValue, float NewValue)
{
#if WITH_SERVER_CODE
	if (AbilitySystemComponent && DamageEffectSpec)
	{
