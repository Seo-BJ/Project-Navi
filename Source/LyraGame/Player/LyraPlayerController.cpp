...
void ALyraPlayerController::ServerRequestSetTeamId_Implementation(int32 NewTeamID)
{
#if WITH_SERVER_CODE
	AGameStateBase* GameState = GetWorld()->GetGameState();
	if (!IsValid(GameState))
	{
		UE_LOG(LogLyraTeams, Error, TEXT("ServerRequestSetTeam: Failed to get GameState."));
		return;
	}
	
	ULyraTeamCreationComponent* TeamComponent = GameState->FindComponentByClass<ULyraTeamCreationComponent>();
	if (!IsValid(TeamComponent))
	{
		UE_LOG(LogLyraTeams, Error, TEXT("ServerRequestSetTeam: Failed to find ULyraTeamCreationComponent on GameState."));
		return;
	}
...
void ALyraPlayerController::ServerRequestSetIndexInTeam_Implementation(int32 NewIndexInTeam)
{
#if WITH_SERVER_CODE
	AGameStateBase* GameState = GetWorld()->GetGameState();
	if (!IsValid(GameState))
	...
	if (!IsValid(TeamComponent))
	{
		UE_LOG(LogLyraTeams, Error, TEXT("ServerRequestSetTeam: Failed to find UNaviTeamCreationComponent on GameState."));
		return;
	}
