...
void ALyraGameMode::InitGameLift()
{
	...
	{
		UDS_GameInstanceSubsystem* DS_GameInstanceSubsystem = GameInstance->GetSubsystem<UDS_GameInstanceSubsystem>();
		if (IsValid(DS_GameInstanceSubsystem))
		{
			FServerParameters ServerParameters;
			SetServerParameters(ServerParameters);
			DS_GameInstanceSubsystem->InitGameLift(ServerParameters);
		}
	}
}
void ALyraGameMode::SetServerParameters(FServerParameters& OutServerParameters)
{
	//AuthToken returned from the "aws gamelift get-compute-auth-token" API. Note this will expire and require a new call to the API after 15 minutes.
	if (FParse::Value(FCommandLine::Get(), TEXT("-authtoken="), OutServerParameters.m_authToken))
	{
		UE_LOG(LogLyraGameLift, Log, TEXT("AUTH_TOKEN: %s"), *OutServerParameters.m_authToken)
	}

	//The Host/compute-name of the GameLift Anywhere instance.
	if (FParse::Value(FCommandLine::Get(), TEXT("-hostid="), OutServerParameters.m_hostId))
	{
		UE_LOG(LogLyraGameLift, Log, TEXT("HOST_ID: %s"), *OutServerParameters.m_hostId)
	}

	//The Anywhere Fleet ID.
	if (FParse::Value(FCommandLine::Get(), TEXT("-fleetid="), OutServerParameters.m_fleetId))
	{
		UE_LOG(LogLyraGameLift, Log, TEXT("FLEET_ID: %s"), *OutServerParameters.m_fleetId)
	}

	//The WebSocket URL (GameLiftServiceSdkEndpoint).
	if (FParse::Value(FCommandLine::Get(), TEXT("-websocketurl="), OutServerParameters.m_webSocketUrl))
	{
		UE_LOG(LogLyraGameLift, Log, TEXT("WEBSOCKET_URL: %s"), *OutServerParameters.m_webSocketUrl)
	}
...
void ALyraGameMode::RemovePlayerSession(AController* Exiting)
{
	ALyraPlayerController* PlayerController = Cast<ALyraPlayerController>(Exiting);
	if (!IsValid(PlayerController)) return;
	ALyraPlayerState* PlayerState = Cast<ALyraPlayerState>(PlayerController->PlayerState);
