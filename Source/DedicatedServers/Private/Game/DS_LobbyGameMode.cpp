...
void ADS_LobbyGameMode::PreLogin(const FString& Options, const FString& Address, const FUniqueNetIdRepl& UniqueId, FString& ErrorMessage)
{
	...
	if (!GetWorld()->IsPlayInEditor()) // <--- PIE 확인 추가
	{
		...
		UE_LOG(LogDedicatedServers, Warning, TEXT("ADS_LobbyGameMode::PreLogin- PlayerSessionId: %s, Username: %s"), *PlayerSessionId, *Username);
	}
	else
	{
		ErrorMessage = TEXT("");
		UE_LOG(LogDedicatedServers, Warning, TEXT("ADS_LobbyGameMode::PreLogin (PIE) - Skipping GameLift validation."));
	}
}
