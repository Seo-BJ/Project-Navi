...
struct FNaviDamageStatics
{
	...
	FNaviDamageStatics()
	{
		...
		ShieldDef = FGameplayEffectAttributeCaptureDefinition(ULyraHealthSet::GetShieldAttribute(), EGameplayEffectAttributeCaptureSource::Target, true);
	}
};
...
UNaviDamageExecution::UNaviDamageExecution()
{
	...
	RelevantAttributesToCapture.Add(NaviDamageStatics().ShieldDef);
}
...
void UNaviDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	if (AdjustedDamage > 0.0f)
	{
		...
		{
			if (TargetShield > AdjustedDamage)
			{
				DamageToShield = AdjustedDamage;
			}
			else
			{
				DamageToShield = TargetShield;
				DamageToHealth =  FMath::Max(0.0f,(AdjustedDamage - TargetShield));
			}
		}
	}
	...
	{
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(ULyraHealthSet::GetIncomingDamageToShieldAttribute(), EGameplayModOp::Additive, DamageToShield));
	}
